...
app.post("/api/admin/fix-archived-data", requireAdmin, async (req, res) => {
  try {
    ...
    for (const fileName of historyFiles) {
      ...
      // 从文件名提取userId: history-{userId}.json
      const userId = fileName.replace('history-', '').replace('.json', '');
        ...
        for (const item of history) {
          ...
          if (!item.imageUrl && item.fileName) {
            ...
            try {
              ...
              // 文件不存在，无法恢复
              console.log(`  ❌ 文件不存在: ${possibleImagePath}`);
            }
          }
        }
...
7) 结构保持清晰，可分行描述核心要素，结尾可添加渲染/标签（如“official emblem design, minimalistic vector style, balanced composition, high resolution, crisp lines, neutral lighting”）。`;
    ...
    if (mode === 'edit') {
      systemPrompt += `\n\n⚠️ 特别注意（图像编辑模式）：
- 用户已上传1张参考图片（称为"图片1"或"参考图"）
- 所有对图片的描述都是针对这张图片
- 如果涉及人物编辑（服装、姿势、背景等），必须强调：
  "图片1中人物的面部特征、五官细节、神态表情必须与原图保持完全一致"
- 只改变用户要求的部分，不改变人物容貌
- 使用"图片1"来明确指代参考图`;
    }
    ...
    if (mode === 'compose') {
      systemPrompt += `\n\n⚠️ 特别注意（图像合成模式）：
- 用户已上传多张参考图片（图片1、图片2、图片3...）
- 需要根据用户的描述判断哪些图片包含人物
- 如果用户提到具体图片编号（如"图片1的人物"），在优化时明确指出：
  "图片X中人物的面部特征、五官、表情必须与原图完全一致"
- 如果用户没有明确指明图片编号，但可能涉及人物，添加通用提醒：
  "如果图片中有人物，请保持各自的面部特征和容貌不变"
- 对于多个人物，分别标注：
  "图片1的人物保持容貌一致，图片2的人物保持容貌一致"`;
    }
...
优化输出：一只优雅的波斯猫，柔软的白色长毛，琥珀色的眼睛，坐在阳光洒落的窗台上。柔和的自然光从侧面照射，营造出温暖的氛围。背景虚化，突出猫咪的细节。高清摄影，浅景深，专业人像级别的画质。`;
    ...
    if (mode === 'edit') {
      systemPrompt += `\n\n图像编辑示例：
用户输入：换一身和服
优化输出：为图片1中的人物更换传统日式和服，采用精美的花纹图案和鲜艳的色彩。⚠️ 重要：图片1中人物的面部特征、五官细节、神态表情必须与原图保持完全一致，只改变服装，不改变容貌。背景和光线保持与原图协调。

用户输入：把背景换成山景
优化输出：保持图片1中的人物不变，将背景替换为壮丽的山景，远处是连绵的雪山，近处是青翠的树林。⚠️ 重要：图片1中人物的面部特征、身材比例、姿态必须与原图完全一致，只改变背景环境。自然光线，整体协调。`;
    }
    
    if (mode === 'compose') {
      systemPrompt += `\n\n图像合成示例1（明确指定图片）：
用户输入：图片1的人物穿图片2的衣服
优化输出：将图片1中人物的服装替换为图片2中的服装样式。⚠️ 重要：图片1中人物的面部特征、五官、表情必须与原图完全一致，只改变服装。确保服装合成自然，光影协调。

示例2（涉及人物但未明确）：
用户输入：把两张图合成一个场景
优化输出：将两张图片中的元素合成到统一场景中，构图协调，光线统一。⚠️ 如果图片中有人物，请保持各自的面部特征和容貌不变，确保人物形象不变形不走样。

示例3（明确多个人物）：
用户输入：图片1的人和图片2的人站在一起
