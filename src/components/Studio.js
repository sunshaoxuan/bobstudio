...
import { Upload, Image, Edit3, Layers, Download, Settings, Sparkles, X, Home, LogOut, BarChart3, Eye, EyeOff, Shield } from 'lucide-react';
...
const Studio = () => {
  ...
  const loadImageHistory = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ 已加载用户 ${currentUser.id} 的历史记录: ${historyData.length} 张图片`);
      } else if (response.status === 404) {
        console.log(`ℹ️ 用户 ${currentUser.id} 的历史记录文件不存在，设置为空数组`);
        setImageHistory([]);
      } else {
  ...
  const saveHistoryToServer = useCallback(async (historyData, userId) => {
    try {
      console.log('💾 保存历史记录到服务器，用户:', userId, '图片数量:', historyData.length);
  ...
  useEffect(() => {
    console.log('🔄 currentUser useEffect触发，currentUser:', currentUser?.id, 'previousUserId:', previousUserId);
    ...
    if (currentUser) {
      ...
      if (previousUserId !== currentUser.id) {
        console.log('📥 需要加载历史记录，从', previousUserId, '到', currentUser.id);
        loadImageHistory();
      } else {
  ...
  useEffect(() => {
    ...
    if (imageHistory.length > 0) {
      console.log('📋 历史记录详情:', imageHistory.map(item => ({ id: item.id, createdAt: item.createdAt })));
    }
  ...
  const analyzeRefusalPatterns = (text) => {
    ...
    // 检查"没找到"相关模式
    if (text.includes('没有找到') || text.includes('找不到') || text.includes('not found') || 
        text.includes('没找到') || text.includes('未找到')) {
      reasons.push('• 使用"没找到"模式委婉拒绝');
      isRefusal = true;
    }

    // 检查"没有合适的"模式
    if (text.includes('没有合适的') || text.includes('没有适合的') || text.includes('no suitable')) {
      reasons.push('• 使用"没有合适的"模式委婉拒绝');
      isRefusal = true;
    }

    // 检查"无法提供"模式
    if (text.includes('无法提供') || text.includes('不能提供') || text.includes('cannot provide')) {
      reasons.push('• 使用"无法提供"模式委婉拒绝');
      isRefusal = true;
    }

    // 检查包含敏感词但假装正常的模式
    const sensitiveWords = ['美女', '性感', '暴露', '裸体', '淘边', '躺着', '色情'];
    ...
    // 检查奇怪的描述模式
    if (/[0-9]+地[0-9]+/.test(text) || text.includes('向下躺着') || (text.includes('上方') && text.includes('向下'))) {
      reasons.push('• 使用奇怪的描述语言来掩盖拒绝');
      isRefusal = true;
    }

    // 检查文本长度异常短
    if (text.length < 50 && (text.includes('照片') || text.includes('图片') || text.includes('image'))) {
    ...
    if (data.candidates && data.candidates.length > 0) {
      ...
      if (candidate.finishReason === 'SAFETY') {
        ...
        if (candidate.safetyRatings) {
          ...
          if (blockedRatings.length > 0) {
            const categories = blockedRatings.map(r => {
              switch (r.category) {
                case 'HARM_CATEGORY_HARASSMENT': return '骚扰内容';
                case 'HARM_CATEGORY_HATE_SPEECH': return '仇恨言论';
                case 'HARM_CATEGORY_SEXUALLY_EXPLICIT': return '成人内容';
                case 'HARM_CATEGORY_DANGEROUS_CONTENT': return '危险内容';
      ...
      if (candidate.finishReason && candidate.finishReason !== 'STOP') {
        switch (candidate.finishReason) {
          case 'IMAGE_SAFETY':
            title = '图像安全检查';
            message = '生成的图像内容被安全检查系统拦截，无法完成生成\n\n💡 说明：\n• 这是专门针对图像内容的安全检查\n• 即使文本描述看似正常，生成的图像可能触发安全规则\n• 建议尝试更温和、更抽象的描述词语';
            break;
          case 'MAX_TOKENS':
            title = '响应长度限制';
      ...
      if (candidate.content && candidate.content.parts) {
        ...
        if (textParts.length > 0) {
          ...
          if (responseText.toLowerCase().includes('cannot') || responseText.toLowerCase().includes('unable')) {
            message = 'API明确表示无法处理您的请求';
          } else if (responseText.toLowerCase().includes('safety') || responseText.toLowerCase().includes('policy')) {
            ...
            title = '疑似内容拒绝';
            message = 'API返回了文本响应但没有生成图像，这通常表示内容可能不被允许\n\n💡 建议：\n• 尝试修改描述词语\n• 避免可能敏感的内容\n• 使用更抽象或艺术化的表达';
          }
  ...
  const generateFileName = useCallback(() => {
    ...
    const nextCount = typeof countBase === 'number' && !Number.isNaN(countBase)
      ? countBase + 1
      : 1;
  ...
  // 自动保存图片到本地
  // 保存图片到会话历史记录（内存存储）
  const saveImageToHistory = useCallback(async (imageUrl, prompt, mode) => {
  ...
  // 删除单条会话历史记录
  const deleteHistoryImage = useCallback((imageId) => {
    setImageHistory(prev => {
  ...
  const processFiles = useCallback((files) => {
    ...
    if (imageFiles.length === 0) {
      showError('文件类型错误', '请选择图片文件（支持 JPG、PNG、GIF、WebP 等格式）');
      return;
    }
    ...
    console.log(`已上传${imageFiles.length}张图片，当前总数：${uploadedImages.length + imageFiles.length}`);
  ...
  const callAPI = async (requestBody) => {
    try {
      ...
      throw new Error(`${errorInfo.title}: ${errorInfo.message}`, { cause: errorInfo });
    } catch (error) {
  ...
  }, [mode, uploadedImages.length, errorModal.show, processFiles, closeErrorModal]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-100 to-blue-100">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        {currentUser?.showApiConfig ? (
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-700">API配置</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                >
                  ...
                  <p className="text-gray-600">
                    {mode === 'edit' ? '上传要编辑的图像' : '上传多张图像进行合成'}
                  </p>
                  {mode === 'compose' && (
                    <p className="text-sm text-gray-500 mt-1">支持选择多张图片</p>
                  )}
            ...
            <button
              ...
              disabled={loading || !apiKey || !prompt || ((mode === 'edit' || mode === 'compose') && uploadedImages.length === 0)}
            ...
            >
              {loading ? (
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  {mode === 'generate' ? '生成图像' : mode === 'edit' ? '编辑图像' : '合成图像'}
                </div>
              )}
            </button>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mt-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold flex items-center gap-2">
                📸 本次会话记录
                <span className="text-sm text-gray-500">({imageHistory.length}/20)</span>
              </h3>
            ...
            {imageHistory.length > 0 ? (
                  ...
                  {imageHistory.map((record) => (
                    <div key={record.id} className="relative group">
                      ...
                      <div className="mt-2 text-xs text-gray-600 text-center">
                        <div className="font-medium text-blue-600">#{imageHistory.length - imageHistory.indexOf(record)}</div>
                        <div className="text-gray-500">{new Date(record.createdAt).toLocaleString('zh-CN', { 
                          month: '2-digit', 
                          day: '2-digit', 
                          hour: '2-digit', 
                          minute: '2-digit'
              ...
              <div className="text-center text-gray-500 py-8">
                ...
                <p>还没有生成任何图片</p>
                <p className="text-sm mt-2">生成图片后会自动保存到服务器，历史记录会持久保存</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h2 className="text-lg font-semibold mb-4 text-blue-800">✨ 功能说明</h2>
